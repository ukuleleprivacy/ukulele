...
const formatBalance = (balance: ethers.BigNumber) => {
  const tokenAmount = Number(ethers.utils.formatUnits(balance, ukuleleToken.decimals));
...
export const WalletBalance = ({ fullWidth = false }: WalletBalanceProps) => {
  ...
  useEffect(() => {
    ...
    let isCurrent = true;
    const tokenContract = new ethers.Contract(ukuleleToken.address, balanceAbi, library);
    ...
    const loadBalance = async () => {
      try {
        ...
        console.warn('[Wallet] Could not load the public UKULELE balance.', error);
  ...
  const displayBalance = hasError ? 'Unavailable' : balance === null ? 'Loading…' : `${balance} UNK`;

  return (
    <Tooltip title="Public UKULELE token balance" arrow>
      <Box
        role="status"
        aria-label={`Public UKULELE balance: ${displayBalance}`}
        sx={{
          minHeight: 40,
